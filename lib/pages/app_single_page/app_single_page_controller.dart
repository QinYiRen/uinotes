...
class AppSinglePageController extends GetxController {
  ...
  getDataList({bool reset = false, uuid}) async {
    var newList = <Result>[];
    try {
      ...
      var res = await RequestRepository.appDetailApi(uuid, page.value);

      bool isLogin = false;
      bool isVip = false;

      if (res.data != null && res.statusCode != 204) {
        var data = AppDetailModel.fromJson(res.data);
  ...
  getDataNum(uuid) async {
    var res = await RequestRepository.appDetailCountApi(uuid);
    num.value = res as int;
  }
