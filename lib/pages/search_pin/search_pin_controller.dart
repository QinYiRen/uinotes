...
class SearchPinController extends GetxController {
  ...
  getDataList({bool reset = false, wd}) async {
    ...
    try {
      ...
      var res = await RequestRepository.searchApi(wd, 'pin', page.value);
      bool isLogin = false;
      bool isVip = false;

      if (res.data != null && res.statusCode != 204) {
        var data = SearchModel.fromJson(res.data);
      ...
      if (res.statusCode == 204) {
        isVip = pref.get('isVip') == 1 ? true : false;
        isLogin = pref.containsKey('isVip')? true : false;
